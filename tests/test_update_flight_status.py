...
def test_scheduled_to_scheduled(client):
    flight_number = "FL123"
    response = client.put(f"/flights/?flight_number={flight_number}&flight_status=Scheduled")
    assert response.status_code == 200
    assert response.json()["status"] == "Scheduled"

def test_scheduled_to_awaiting_boarding(client):
    flight_number = "FL123"
    response = client.put(f"/flights/?flight_number={flight_number}&flight_status=Awaiting%20Boarding")
    assert response.status_code == 200
    assert response.json()["status"] == "Awaiting Boarding"

def test_scheduled_to_boarding(client):
    flight_number = "FL123"
    response = client.put(f"/flights/?flight_number={flight_number}&flight_status=Boarding")
    assert response.status_code == 422
    assert response.json() == {"detail": "Flight was not in Awaiting Boarding status"}

def test_scheduled_to_departing(client):
    flight_number = "FL123"
    response = client.put(f"/flights/?flight_number={flight_number}&flight_status=Departing")
    assert response.status_code == 422
    assert response.json() == {"detail": "Flight was not in Boarding status"}

def test_scheduled_to_departed(client):
    flight_number = "FL123"
    response = client.put(f"/flights/?flight_number={flight_number}&flight_status=Departed")
    assert response.status_code == 422
    assert response.json() == {"detail": "Flight was not in Departing status"}

def test_scheduled_to_delayed(client):
    flight_number = "FL123"
    response = client.put(f"/flights/?flight_number={flight_number}&flight_status=Delayed")
    assert response.status_code == 200
    assert response.json()["status"] == "Delayed"

def test_scheduled_to_cancelled(client):
    flight_number = "FL123"
    response = client.put(f"/flights/?flight_number={flight_number}&flight_status=Cancelled")
    assert response.status_code == 200
...
def test_awaiting_boarding_to_awaiting_boarding(client):
    ...
    assert response.status_code == 200
    assert response.json()["status"] == "Awaiting Boarding"
    client.put(f"/flights/?flight_number={flight_number}&flight_status=Awaiting%20Boarding")
    response = client.put(f"/flights/?flight_number={flight_number}&flight_status=Awaiting%20Boarding")
    assert response.status_code == 200
    assert response.json()["status"] == "Awaiting Boarding"
...
def test_invalid_status(client):
    flight_number = "FL123"
    response = client.put(f"/flights/?flight_number={flight_number}&flight_status=InvalidStatus")
    assert response.status_code == 422
